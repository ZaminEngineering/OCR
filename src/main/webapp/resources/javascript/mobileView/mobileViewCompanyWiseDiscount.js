...
myApp.controller('myCtrl', function($scope, $http) {
    	...
    	$(".fetchExistingExpenses").click(function(){
    	   ...
    	   if(date!="" && company!="" && company != undefined){
    		    ...
    		    var myUrl = window.location.href;
    	    	var url = myUrl.substring(0, myUrl.lastIndexOf('/') + 1);
    	    	var transactionUrl = url+"mobileView/getDiscountCompanyWiseForAllMonth?date="+date+"&company="+company;
    	    	$http({
    	    	    method : "GET",
    	    	    url : transactionUrl
    	    	  }).then(function mySuccess(response) {
     	    	...
     	    	var receivedUrl = url+"mobileView/getDiscountCompanyWiseForAllMonthWithReceived?company="+company;
     	    	$http({
     	    	    method : "GET",
     	    	    url : receivedUrl
     	    	  }).then(function mySuccess(response) {
     	    	...
     	    	var rentalUrl = url+"mobileView/getUpToMonthRental?date="+date+"&company="+company;
     	    	$http({
     	    	    method : "GET",
     	    	    url : rentalUrl
     	    	  }).then(function mySuccess(response) {
